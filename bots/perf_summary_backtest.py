# bots/perf_summary_backtest.py
# Calcul des performances pour le backtest (signaux historiques 2 ans)
...
from config import LOG_PATH_BACKTEST, OUT_PATH_BACKTEST, STRATEGY_KEYS
...
logger = logging.getLogger("perf_summary_backtest")
...
def main():
    signals = load_signals_log(LOG_PATH_BACKTEST)
    if not signals:
        logger.info("Aucun signal dans le log backtest. Rien à faire.")
        empty_summary = {
        ...
        save_json(empty_summary, OUT_PATH_BACKTEST)
        return
    ...
    save_signals_log(updated_signals, LOG_PATH_BACKTEST)
    ...
    save_json(summary, OUT_PATH_BACKTEST)
    logger.info("Backtest performance summary updated.")
